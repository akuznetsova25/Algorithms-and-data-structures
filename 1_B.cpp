...
public:
	segment_tree(std::vector<int>& foliage) :
		...
		{
			tree[j] = std::gcd(tree[2 * j + 1], tree[2 * j + 2]);
		}
	}


	int great_common_divisor(int left, int right) {
		left += initial_size - 2;
		...
		while (left < right) {
			if (left == Right(Parent(left))) {
				left_res = std::gcd(left_res, tree[left]);
			}
			left = Parent(left + 1);
			if (right == Left(Parent(right))) {
				right_res = std::gcd(right_res, tree[right]);
			}
	...
	void update(int i, int new_value) {
		...
		do {
			i = Parent(i);
			tree[i] = std::gcd(tree[Left(i)], tree[Right(i)]);
		} while (i != 0);
	}
};
...
int main()
{
	...
	{
		if (query[i] == 's')
			std::cout << st.great_common_divisor(query_values[i].first, query_values[i].second) << " ";
		if (query[i] == 'u') {
